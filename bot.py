...
def apply_initial_sound_law(char):
    """ 
    두음 법칙 적용 함수 
    요청하신 대로 '러' 계열(초성이 'ㄹ'이고 모음이 'ㅓ'인 글자)은 절대 변하지 않고 그대로 유지되며,
    '래' 계열(vowel == 1) 및 '뢰' 계열(vowel == 11)은 초성이 'ㄴ'으로, '례' 계열(vowel == 7)은 초성이 'ㅇ'으로 종성(받침)에 상관없이 완벽하게 교정됩니다.
    """
    if not char: return char
    
    # 한글 유니코드 분석 (초성, 중성, 종성 분리)
    ...
    # 초성이 'ㄹ'(lead == 5)이고 중성이 'ㅓ'(vowel == 4)인 경우 -> '러' 계열 전체 (럭, 런, 렁 등 포함)
    # 이 경우 두음 법칙을 절대 적용하지 않고 원본 글자 그대로 반환합니다.
    if lead == 5 and vowel == 4:
        return char
        
    # [수정 적용] 초성이 'ㄹ'(lead == 5)인 경우 처리
    if lead == 5:
        # 'ㅐ' 모음(vowel == 1) -> 초성을 'ㄴ'(lead = 2)으로 변경 (예: 래/랭 -> 내/냉)
        if vowel == 1:
            lead = 2
        # 'ㅖ' 모음(vowel == 7) -> 초성을 'ㅇ'(lead = 11)으로 변경 (예: 례/롄 -> 예/옌)
        elif vowel == 7:
            lead = 11
        # [수정 사항] 'ㅚ' 모음(vowel == 11) -> 초성을 'ㄴ'(lead = 2)으로 변경 (예: 뢰/룅/룀 -> 뇌/뇡/뇜)
        elif vowel == 11:
            lead = 2
        elif vowel in [0, 13, 18]: # ㅏ(0), ㅜ(13), ㅡ(18) -> 'ㄴ'(2)으로 변경 (예: 란->난, 루->누)
            lead = 2
        elif vowel in [2, 12, 14, 20]: # ㅑ(2), ㅛ(12), ㅜ/ㅠ 계열(14), ㅣ(20) -> 'ㅇ'(11)으로 변경 (예: 린->인)
            lead = 11
            
    # 초성이 'ㄴ'(lead == 2)인 경우 처리
    elif lead == 2:
        if vowel in [4, 12, 14, 20]: # ㅓ(4), ㅛ(12), ㅠ(14), ㅣ(20) -> 'ㅇ'(11)으로 변경 (예: 녀->여, 니->이)
            lead = 11
    else: 
        return char
...
async def on_message(message):
    ...
    if channel_id in active_games:
        ...
        if is_my_turn:
            word = msg_content
            if not word: return
            if word not in WORD_DICT: return
