...
def apply_initial_sound_law(char):
    """ 
    두음 법칙 적용 함수 
    요청하신 규칙에 따라 종성(받침) 및 단어 내 위치에 관계없이 아래와 같이 완벽하게 변환합니다.
    ...
    if lead == 5:
        ...
        if vowel in [0, 1, 8, 11, 13, 18]:
            lead = 2
        # ㅑ(2), ㅓ(4), ㅖ(7), ㅛ(12), ㅠ(14), ㅣ(20) -> 초성 'ㅇ'(lead = 11)으로 변경
        elif vowel in [2, 4, 7, 12, 14, 20]:
            lead = 11
            
    # 초성이 'ㄴ'(lead == 2)인 경우 처리
    elif lead == 2:
        # ㅓ(4), ㅛ(12), ㅠ(14), ㅣ(20) -> 초성 'ㅇ'(lead = 11)으로 변경 (녀, 뇨, 뉴, 니 -> 여, 요, 유, 이)
        if vowel in [4, 12, 14, 20]:
            lead = 11
    else: 
        return char
