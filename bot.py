...
def apply_initial_sound_law(char):
    """ 
    두음 법칙 적용 함수 
    요청하신 진짜 규칙에 따라 완벽하게 변환합니다.
    - 오직 '러' 계열(모음 ㅓ)만 예외적으로 절대 변환되지 않고 그대로 유지됩니다.
    - '려', '녀' 계열(모음 ㅕ)은 정상적으로 '여'로 변환됩니다.
    - 라, 래, 로, 뢰, 루, 르 계열 ➔ 나, 내, 노, 뇌, 누, 느 (초성 'ㄹ' -> 'ㄴ')
    - 랴, 려, 례, 료, 류, 리 계열 ➔ 야, 여, 예, 요, 유, 이 (초성 'ㄹ' -> 'ㅇ')
    - 녀, 뇨, 뉴, 니 계열 ➔ 여, 요, 유, 이 (초성 'ㄴ' -> 'ㅇ')
    """
    ...
    # 초성이 'ㄹ'(lead == 5)인 경우 처리
    if lead == 5:
        # ㅏ(0), ㅐ(1), ㅗ(8), ㅚ(11), ㅜ(13), ㅡ(18) -> 초성 'ㄴ'(lead = 2)으로 변경
        if vowel in [0, 1, 8, 11, 13, 18]:
            lead = 2
        # ㅑ(2), ㅕ(6), ㅖ(7), ㅛ(12), ㅠ(17), ㅣ(20) -> 초성 'ㅇ'(lead = 11)으로 변경
        # (모음 ㅓ(4)는 절대 변환 리스트에 포함시키지 않습니다.)
        elif vowel in [2, 6, 7, 12, 17, 20]:
            lead = 11
            
    # 초성이 'ㄴ'(lead == 2)인 경우 처리
    elif lead == 2:
        # ㅕ(6), ㅛ(12), ㅠ(17), ㅣ(20) -> 초성 'ㅇ'(lead = 11)으로 변경
        # (모음 ㅓ(4)는 절대 변환 리스트에 포함시키지 않습니다.)
        if vowel in [6, 12, 17, 20]:
            lead = 11
    else: 
        return char
    ...
    if channel_id in active_games:
        ...
        if is_my_turn:
            ...
            if game["last_word"]:
                ...
                # 입력단어의 첫 글자가 원본 마지막 글자와 같거나, 마지막 글자에 두음 법칙을 적용한 글자와 같아야 인정
